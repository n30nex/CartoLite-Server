...
func run() error {
	...
	broker.Start(ctx)
	api, err := httpapi.New(state, hub, broker.Ready, cfg.Version, cfg.GitSHA, httpapi.PublicBasemapConfig{Provider: cfg.MapProvider, CartoBrowserKey: cfg.CartoBrowserKey})
	if err != nil {
		stop()
		state.Wait()
		return err
