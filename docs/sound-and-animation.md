...
CartoLite offers [30 packet sound voices](sound-voices.md) using native Web Audio with no samples or audio dependency. All 30 sound voices use cached `PeriodicWave` definitions with stable packet, route, hop, and scene variation. Every route hop crossing the current viewport schedules one articulation and one oscillator. Off-screen hops and observer-only traffic stay silent.
...
Each live hop follows its confirmed geographic segment. In 3D, trails and residue follow sampled terrain while ground rings respond to the camera; historical routes drape over the same terrain. Topo turns on with 3D, with adjustable terrain height (1x by default). Date-line strokes split at the world-map seam without creating extra hops or notes. Its cue uses a sharp packet core, short glow, restrained sparks, relay handoff, destination shimmer, node wake, and configurable 0–45-second residue (15 seconds in Crisp). Hiding Routes does not hide recent live traffic.
